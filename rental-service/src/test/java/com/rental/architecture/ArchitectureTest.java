...
class ArchitectureTest {
    ...
    // -------------------------------------------------------------------------
    // Metoda 1: warstwy heksagonalne i ich zależności
    // -------------------------------------------------------------------------
    ...
    void shouldMaintainHexagonalLayers() {
        ...
        Architectures.LayeredArchitecture layerRule = layeredArchitecture()
                ...
                // application może używać domain i ports
                .whereLayer("application").mayOnlyAccessLayers("domain", "ports")
                // ports może używać domain
                .whereLayer("ports").mayOnlyAccessLayers("domain")
                // adapters mogą używać application, ports i domain
                .whereLayer("adapters").mayOnlyAccessLayers("application", "ports", "domain")
                // domain jest absolutnie izolowana — nic nie może do niej "wciągnąć"
                .whereLayer("domain").mayNotAccessAnyLayer();

        // when + then
        layerRule.check(importedClasses);

        // given — dodatkowa weryfikacja: brak Spring i Hibernate w domain
    ...
    // -------------------------------------------------------------------------
    // Metoda 2: adaptery driven komunikują się przez ports.out
    // -------------------------------------------------------------------------
        ...
        ArchRule adaptersOutMustImplementPortsOut = classes()
                ...
                .should().implement(
                        com.tngtech.archunit.core.domain.JavaClass.Predicates
                                .resideInAPackage(BASE_PACKAGE + "..ports.out..")
                )
